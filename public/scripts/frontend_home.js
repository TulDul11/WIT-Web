...
async function load_home_alumno(loading_data) {
    ...
    try {
        ...
        data.course_data.forEach((course) => {
            let course_card = `<div class="card"  style="width: 18rem; background-color: #ffffff; border-radius: 0.5rem; box-shadow: 0 0 12px rgba(1, 28, 44, 0.3), 0 0 22px rgba(0, 163, 255, 0.2);">
...
async function load_home_profesor(loading_data) {
    ...
    try {
        ...
        data.course_data.forEach((course) => {
            let course_tag = `<a href="/course?code=${course[0].cod}">
                                    <div class="sidebar_courses_item">
                                        <span class="sidebar_courses_item_text">
                                                ${course[0].cod}. ${course[0].nombre}
                                        </span>
                                    </div>
                                </a>`
